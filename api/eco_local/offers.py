...
def _resolve_user_business_id(
    ...
    if requested_business_id:
        ...
        if not rec:
            raise HTTPException(
                status_code=403, detail="You don't have access to that business"
            )
    ...
    if len(ids) == 0:
        raise HTTPException(
            status_code=404,
            detail="You don't have a business yet or you arent a business!",
        )
    if len(ids) == 1:
        return ids[0]
    raise HTTPException(
        status_code=400,
        detail={
            "message": "Multiple businesses found; specify ?business_id=...",
            "your_business_ids": ids,
        },
    )
...
def _assert_voucher_belongs_to_user_business(
    s: Session, user_id: str, voucher_code: str
) -> Dict[str, Any]:
    rec = s.run(
        f"""
        MATCH (u:User {{id:$uid}})-[{_OWNS_EDGES[0]}]->(b:BusinessProfile)
    ...
    if not rec:
        raise HTTPException(
            status_code=404, detail="Voucher not found or not for your business"
        )
    return {
        "code": rec["code"],
        "status": rec["status"],
...
class OfferIn(BaseModel):
    ...
    eco_price: int = Field(..., ge=1)
    fiat_cost_cents: int = Field(0, ge=0)
...
def _is_visible(o: Dict[str, Any]) -> bool:
    ...
    if vu:
        try:
            d = (
                datetime.fromisoformat(vu).date()
                if "T" in vu
                else date.fromisoformat(vu)
            )
            if d < date.today():
                return False
        except Exception:
...
def create_offer_api(
    ...
    business_id: Optional[str] = Query(
        None, description="Optional if you own multiple"
    ),
    s: Session = Depends(session_dep),
    user_id: str = Depends(current_user_id),
):
...
# ============================================================
# Vouchers (owner-side verify/consume)
# ============================================================
...
def verify_voucher_api(
    ...
    if meta["expiresAt"] and now > meta["expiresAt"] and meta["status"] in (
        "issued",
        "verified",
    ):
        s.run(
        ...
        return VoucherVerifyOut(
            ...
            offer={
                "id": meta["offer_id"],
                "title": meta["offer_title"],
                "eco_price": meta["eco_price"],
            },
            status="expired",
            expires_in_sec=0,
        )
    ...
    return VoucherVerifyOut(
        ...
        status="verified"
        if meta["status"] in ("issued", "verified")
        else meta["status"],
...
def suggest_offer_price_api(
    ...
    if fiat_cents == 0 and (body.type == "discount") and (body.percent is not None):
        avg_basket = int(body.avg_basket_cents or 2000)
...
def _csv_to_list(val: Optional[str]) -> List[str]:
    if not val:
        return []
    return [p.strip() for p in val.split(",") if p.strip()]


@router.get("/places", response_model=PlacesResponseOut)
def public_places_with_offers(
    ...
    q: str | None = Query(None, description="Search on name or tags"),
    pledge: str | None = Query(
        None,
        description="Comma-separated pledge tiers: starter,builder,leader",
    ),
    industry: str | None = Query(
        None,
        description="Comma-separated industry_group values",
    ),
    area: str | None = Query(
        None,
        description="Comma-separated area values",
    ),
    open_now: bool | None = Query(
        None,
        description="If true, only businesses with open_now=true",
    ),
    ...
    where_clauses = [
        "coalesce(b.visible_on_map, true) = true",
        "b.lat IS NOT NULL",
        "b.lng IS NOT NULL",
    ]
    params: Dict[str, Any] = {}
    ...
    if all(v is not None for v in (min_lat, min_lng, max_lat, max_lng)):
        where_clauses.append(
            "b.lat >= $min_lat AND b.lat <= $max_lat AND "
            "b.lng >= $min_lng AND b.lng <= $max_lng"
        )
        params.update(
            dict(min_lat=min_lat, min_lng=min_lng, max_lat=max_lat, max_lng=max_lng)
        )

    if q:
        where_clauses.append(
            "(toLower(b.name) CONTAINS $q OR "
            " any(t IN coalesce(b.tags,[]) WHERE toLower(t) CONTAINS $q))"
        )
        params["q"] = q.lower()

    pledges = _csv_to_list(pledge)
    industries = _csv_to_list(industry)
    areas = _csv_to_list(area)
    ...
    if open_now is True:
        where_clauses.append("coalesce(b.open_now,true) = true")

    where_sql = "WHERE " + " AND ".join(where_clauses) if where_clauses else ""

    distance_expr = (
        "distance(point({latitude:b.lat, longitude:b.lng}), "
    ...
    if sort == "distance":
        ...
        order_by = (
            "ORDER BY (CASE b.pledge_tier "
            "WHEN 'leader' THEN 3 WHEN 'builder' THEN 2 "
            "WHEN 'starter' THEN 1 ELSE 0 END) DESC, b.name ASC"
        )
    else:
