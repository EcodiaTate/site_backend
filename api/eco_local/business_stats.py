# site_backend/api/eco-local/business_stats.py
...
class SummaryOut(BaseModel):
    ...
    offers_total: int
    # Minted by businesses this month = retirements (burns) triggered by offers (BURN_REWARD)
...
def summary(month: str = Query(..., description="YYYY-MM"), s: Session = Depends(session_dep)):
    start_iso, end_iso, start_ms, end_ms = _month_bounds(month)
    ...
    # Retirements in window: BURN_REWARD (settled), paired to offers/business
    rec = s.run(
        """
        MATCH (b:BusinessProfile)-[:REDEEMED]->(t:EcoTx {kind:'BURN_REWARD'})
        ...
        WITH collect(t) AS txs, toInteger(coalesce(sum(toInteger(coalesce(t.amount,t.eco,0))),0)) AS minted, toInteger(count(t)) AS claims
        OPTIONAL MATCH (u:User)-[:SPENT]->(t2:EcoTx) WHERE t2 IN txs
...
def by_industry(month: str = Query(..., description="YYYY-MM"), s: Session = Depends(session_dep)):
    ...
    for r in s.run(
        ...
        OPTIONAL MATCH (b3:BusinessProfile)-[:REDEEMED]->(t:EcoTx {kind:'BURN_REWARD'})
        ...
        WITH industry, businesses, offers,
             toInteger(coalesce(sum(toInteger(coalesce(t.amount,t.eco,0))),0)) AS minted,
             toInteger(coalesce(count(t),0)) AS claims,
             collect(t) AS txs
        OPTIONAL MATCH (u:User)-[:SPENT]->(t2:EcoTx) WHERE t2 IN txs
        RETURN industry, toInteger(businesses) AS businesses, toInteger(offers) AS offers,
               toInteger(coalesce(minted,0)) AS minted, toInteger(coalesce(claims,0)) AS claims,
...
def top_businesses(month: str = Query(..., description="YYYY-MM"),
    ...
    for r in s.run(
        """
        MATCH (b:BusinessProfile)
        OPTIONAL MATCH (b)-[:REDEEMED]->(t:EcoTx {kind:'BURN_REWARD'})
        WHERE coalesce(t.status,'settled')='settled'
          AND toInteger(coalesce(t.createdAt, timestamp(t.at), -1)) >= $start_ms
          AND toInteger(coalesce(t.createdAt, timestamp(t.at), -1)) <  $end_ms
        WITH b,
             ...
             collect(t) AS txs
        OPTIONAL MATCH (u:User)-[:SPENT]->(t2:EcoTx) WHERE t2 IN txs
        WITH b, minted, claims, toInteger(count(DISTINCT u)) AS uniq
        RETURN b.id AS bid,
...
def minted_timeseries(from_month: str = Query(..., description="YYYY-MM"),
    ...
    points: List[Point] = []
    for ym in months:
        _, _, start_ms, end_ms = _month_bounds(ym)
        rec = s.run(
            """
            MATCH (:BusinessProfile)-[:REDEEMED]->(t:EcoTx {kind:'BURN_REWARD'})
            WHERE coalesce(t.status,'settled')='settled'
              AND toInteger(coalesce(t.createdAt, timestamp(t.at), -1)) >= $start_ms
              AND toInteger(coalesce(t.createdAt, timestamp(t.at), -1)) <  $end_ms
            RETURN toInteger(coalesce(sum(toInteger(coalesce(t.amount,t.eco,0))),0)) AS minted
            """,
            {"start_ms": start_ms, "end_ms": end_ms},
        ).single()
