# site_backend/api/auth/refresh.py
...
def _mint_access(uid: str, email: str | None = None) -> tuple[str, int]:
    ...
    payload: dict[str, object] = {"sub": uid, "iat": now, "exp": exp}
    if email:
        payload["email"] = email
    if ACCESS_JWT_ISS:
...
def refresh(request: Request, response: Response):
    ...
    # Issue fresh access token (no email claim here; that's okay for most backend flows)
    access, exp = _mint_access(uid)

    # Also set ACCESS cookie for cookie-based auth (SSR, etc.)
    set_scoped_cookie(
        response,
    ...
    # Frontend refreshOnce() and NextAuth expect this shape
