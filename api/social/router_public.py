...
from .service import (
    ...
    request_friend_by_email,  # ⬅️ NEW import stays at module top
)
...
# ---------- MODELS (local-only) ----------
class RequestByEmailIn(BaseModel):
    email: EmailStr
...
def r_search_friends(
    q: str = Query(min_length=3, description="Name (≥3), or full/prefix ID"),
    session: Session = Depends(session_dep),
    uid: str = Depends(current_user_id),
):
...
# ---------- NEW invite-by-email (privacy-first) ----------
@router.post("/friends/request-by-email")
def r_request_friend_by_email(
    body: RequestByEmailIn,
    session: Session = Depends(session_dep),
    uid: str = Depends(current_user_id),
):
    return request_friend_by_email(session, uid, body.email)
